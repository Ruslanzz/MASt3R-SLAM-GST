...
| `no element "nvdsmast3rslam"`, при этом `.so` на месте и `ldd` без «not found»; `gst-inspect-1.0 <путь к .so>` пишет `undefined symbol: ...throw_from_rcl_error...` | образ собран `WITH_ROS2=1`, и линкер (Ubuntu, `--as-needed` по умолчанию) выкинул `librclcpp.so` из `DT_NEEDED` — символ используется только из inline-шаблонов rclcpp. Исправлено в `CMakeLists.txt` (ROS-библиотеки линкуются в обёртке `-Wl,--no-as-needed`). Пересоберите образ. Проверка причины: `gst-inspect-1.0 "$(pkg-config --variable=pluginsdir gstreamer-1.0)/libnvdsmast3rslam.so"` |
