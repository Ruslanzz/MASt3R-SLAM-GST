...
| Сборка `WITH_ROS2=1` падает на cmake: `ModuleNotFoundError: No module named 'ament_package'` | ament_cmake (тянется `find_package(rclcpp)`) требует питон-модуль `ament_package` и проинициализированное ROS-окружение. Уже исправлено в Dockerfile: ставится `python3-ament-package` (он идёт как *recommends*, а мы используем `--no-install-recommends`) и перед `cmake` выполняется `source /opt/ros/humble/setup.bash`. Пересоберите с актуальным Dockerfile |
| `pip install -e` падает: `canonicalize_version() got an unexpected keyword argument 'strip_trailing_zero'` | новый setuptools в образе вызывает API из `packaging>=24`, а установлен более старый `packaging`; при `--no-build-isolation` берётся окружение образа. Исправлено в Dockerfile (`pip install -U "packaging>=24.0"` перед editable-установками). Разовый фикс на месте: `python3 -m pip install -U "packaging>=24.0"` |
| `no element "nvdsmast3rslam"`, при этом `.so` на месте и `ldd` без «not found»; `gst-inspect-1.0 <путь к .so>` пишет `undefined symbol: ...throw_from_rcl_error...` | образ собран `WITH_ROS2=1`, но `librclcpp.so` не попала в `DT_NEEDED`: цель `rclcpp::rclcpp` не тянет саму библиотеку, а Ubuntu линкует `--as-needed`. Исправлено в `CMakeLists.txt` — ROS-зависимости подключаются через `ament_target_dependencies` (+ `--no-as-needed`), для чего в образ добавлен `ros-humble-ament-cmake`. Пересоберите образ. Проверка: `readelf -d "$(pkg-config --variable=pluginsdir gstreamer-1.0)/libnvdsmast3rslam.so" \| grep -i rclcpp` — должна появиться строка `NEEDED librclcpp.so` |
